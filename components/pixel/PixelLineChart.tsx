      ...
      {[0, 25, 50, 75, 100].map((pct) => {
        ...
        return (
          <line key={pct} x1={pad} y1={y} x2={W - pad} y2={y}
            stroke="#C0C0D0" strokeWidth="1" strokeDasharray="4 4" />
        );
      })}
      <path d={pathD} fill="none" stroke="#4088F0" strokeWidth="3" strokeLinejoin="miter" />
      {points.map((p, i) => (
        <rect key={i} x={p.x - 4} y={p.y - 4} width="8" height="8"
          fill="#F8D030" stroke="#101010" strokeWidth="2" />
      ))}
    </svg>
  );
}
